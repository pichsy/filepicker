...
import com.pichs.filepicker.dialog.FilePickerPagingFinalPreviewDialog
import com.pichs.filepicker.dialog.FilePickerPaingPreviewDialog
...
import com.pichs.filepicker.widget.OnFilePickerPagingDragItemTouchHelperCallback
...
class FilePickerPagingFragment : Fragment(), View.OnClickListener {

    private val viewModel by activityViewModels<FilePickerPagingViewModel>()

    private lateinit var binding: FragmentFilepickerHomeBinding

    private var currentTabType = FilePickerSelectType.IMAGE_VIDEO

    private var mPagingGridAdapter: MediaPagingGridAdapter? = null
    private var mPagingLinearAdapter: MediaPagingLinearAdapter? = null
    ...
    companion object {
         ...
         * @param bundle 传递参数
         */
        fun newInstance(): FilePickerPagingFragment {
            return FilePickerPagingFragment()
        }


        private val DIFFER_CALLBACK = object : androidx.recyclerview.widget.DiffUtil.ItemCallback<MediaEntity>() {
    ...
    private fun initListener() {
        FilePickerClickHelper.clicks(binding.llPreview) {
            ...
            FilePickerPagingFinalPreviewDialog(
...
//                Toast.makeText(requireContext(), "确定了 ${resultList?.path}", Toast.LENGTH_SHORT).show()
                    ...
                    if (resultList.isEmpty()) {
                        ...
                        return@FilePickerPagingFinalPreviewDialog
                    }
...
//        }
        ...
        binding.recyclerView.layoutManager = LinearLayoutManager(binding.recyclerView.context, RecyclerView.VERTICAL, false)
        mPagingLinearAdapter = MediaPagingLinearAdapter()
        binding.recyclerView.adapter = mPagingLinearAdapter
...
//        }
        ...
        // 替换为 PagingDataAdapter
        binding.recyclerView.layoutManager = GridLayoutManager(binding.recyclerView.context, 4, GridLayoutManager.VERTICAL, false)
        mPagingGridAdapter = MediaPagingGridAdapter()
        binding.recyclerView.adapter = mPagingGridAdapter
    ...
    inner class MediaPagingGridAdapter : PagingDataAdapter<MediaEntity, PagingGridBindingViewHolder>(DIFFER_CALLBACK) {
        ...
        override fun onBindViewHolder(holder: PagingGridBindingViewHolder, posotion: Int) {
            getItem(posotion)?.let {
                holder.bind(this, it, posotion)
            }
        }
    }
    ...
    inner class PagingGridBindingViewHolder(val itemBinding: FilePickerItemRvAlbumBinding) : RecyclerView.ViewHolder(itemBinding.root) {
        ...
        fun bind(adapter: MediaPagingGridAdapter, item: MediaEntity, position: Int) {
            ...
            if (indexOfSelect != -1) {
                itemBinding.tvSelectIndex.text = "${indexOfSelect + 1}"
                itemBinding.tvSelectIndex.setNormalBackgroundColor(
                    ContextCompat.getColor(
                        itemBinding.tvSelectIndex.context,
                        R.color.file_picker_index_bg_color
                    )
                )
                itemBinding.ivCoverImage.foreground = ContextCompat.getDrawable(itemBinding.tvSelectIndex.context, R.drawable.item_filepicker_select_mask)
                itemBinding.root.isSelected = true
            } else {
                itemBinding.tvSelectIndex.text = ""
                itemBinding.tvSelectIndex.setNormalBackgroundColor(Color.TRANSPARENT)
            ...
            // 判断是否是最后一行
            val lastRowStart = adapter.itemCount - adapter.itemCount % 4

            FilePickerLog.d("FilePickerFragment7777", "modelPosition:$position, lastRowStart:$lastRowStart, itemCount:${adapter.itemCount}")

            if (position >= lastRowStart && (viewModel.getSelectedCount() + viewModel.tempSelectData.size) > 0) {
                itemBinding.clRoot.updateLayoutParams<ViewGroup.MarginLayoutParams> {
                    bottomMargin = XDisplayHelper.dp2px(requireContext(), 80f)
                }
            } else {
            ...
            itemBinding.ivCoverImage.setOnClickListener {
                ...
                if (viewModel.containsSelectedData(item)) {
                    ...
                    adapter.notifyItemChanged(position)
                    updateBottomMenuSelectNumberUI()
                    adapter.notifyItemRangeChanged(lastRowStart, adapter.itemCount - lastRowStart) // 刷新最后一行
                }
            }
...
//                    itemBinding.ivCoverImage.foreground = null
    ...
    private fun initSelectedRecyclerView() {
        ...
        OnFilePickerPagingDragItemTouchHelperCallback(binding.rvSelected.bindingAdapter, viewModel, onDragEnd = {
            updateSelectDataUI()
        }).let { callback ->
    ...
    private fun showFilePickerPreviewDialog(item: MediaEntity) {
        FilePickerPaingPreviewDialog(requireContext(), viewModel, item, onSelect = { item, isSelect, position ->
            // 选择
