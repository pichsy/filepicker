...
class FilePickerPagingActivity : AppCompatActivity() {

    private lateinit var binding: ActivityFilepickerMainBinding

    private val viewModel by viewModels<FilePickerPagingViewModel>()
...
//        viewModel.addSelectedDataList(selectDataList)
        ...
        val fragment = FilePickerPagingFragment.newInstance()
    ...
    private fun checkNavigationBar() {
        if (XNavigationBarUtils.isGestureBarVisible(this)) {
            binding.flContainer.updateLayoutParams<ViewGroup.MarginLayoutParams> {
                bottomMargin = XNavigationBarUtils.getNavigationBarHeight(this@FilePickerPagingActivity)
            }
        } else {
            binding.flContainer.updateLayoutParams<ViewGroup.MarginLayoutParams> {
                bottomMargin = 0
